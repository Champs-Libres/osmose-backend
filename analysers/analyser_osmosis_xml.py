...
class Analyser_Osmosis_From_XML(Analyser_Osmosis):
    ...
    def analyser_osmosis_common(self):
        ...
        for analyser_xml in analysers_dir.iterdir():
            if analyser_xml.is_file():
                # todo faire un clean avant chaque nouvel analyser
                with analyser_xml.open() as xml:
                    xml_analyser = ET.parse(xml)
                    analyser = XMLAnalyser(xml_analyser)
                    ...
                    reqs = [
                        """
CREATE TEMP TABLE IF NOT EXISTS relations_polygon AS
SELECT
    el.id AS id,
...
GROUP BY
    el.id;""",
                    """
CREATE INDEX ON relations_polygon USING GIST (geom);
"""]
                    ...
                    reqs = last_req.to_sql(last=True)
                    for req in reqs[:-1]:
                        self.run(req)
                    self.run(reqs[-1], cb)
